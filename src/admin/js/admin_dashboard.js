(function () {
    const API_ENDPOINT = '../api/get_dashboard_data.php';
    const SEARCH_ENDPOINTS = {
        jobs: '../api/search_jobs.php',
        staff: '../api/search_staff.php',
        clients: '../api/search_clients.php'
    };
    ...
    function normalizeStaffForPanel(member, index) {
        ...
        const positions = Array.isArray(member.positions) && member.positions.length > 0
            ? member.positions
            : tags;
    ...
    function getStaffPanelData() {
        return {
            ...
            staff: dashboardData.staff.map((member, index) => normalizeStaffForPanel(member, index))
        };
    }
    ...
    function getDefaultStaffPanelData() {
        return dashboardData.staff.map((member, index) => normalizeStaffForPanel(member, index));
    }
    ...
    function getDefaultJobsPanelData() {
        return dashboardData.jobs.map(normalizeJobForPanel);
    }
