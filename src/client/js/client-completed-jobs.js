(function () {
    ...
    function goToJobDetails(job) {
        window.location.href = `client-completed-job-details.html?job_id=${encodeURIComponent(job.id)}&from=completed`;
    }
    ...
    function renderJobs(jobs) {
        ...
        if (jobs.length === 0) {
            ...
            jobs.forEach((job) => {
                container.appendChild(createCompletedJobCard(job));
            });
        }
    ...
    initPage().catch((error) => console.warn('Client completed jobs could not load:', error));
}());
